...
The active first-run implementation is structurally complete and heavily
validated, while the remaining product risk is whether movement, combat, builds,
routes, and the boss are enjoyable in continuous play. The roadmap therefore
protects the accepted run/profile/reward architecture and concentrates new work on
one measurable proof path before expanding content.

## Scope / Non-scope

In scope:

- development-only playtest evidence and deterministic fixtures;
- input parity, movement forgiveness, and attack-intent experiments;
- shared enemy openings, three recognizable build directions, one proof stage,
  one revised boss, and the minimum gameplay presentation needed to judge them;
- gated procedural re-entry only after the proof passes.

Non-scope:

- replacing the current three-stage run, persistence, settlement, or authored-room
  owners before an accepted experiment requires it;
- new heroes, classes, active skills, currencies, normal stages, bosses, or broad
  final-art production;
- save-schema changes without explicit owner approval and migration fixtures.

## Assumptions

| Assumption | Evidence / reason | Guard |
| --- | --- | --- |
| The current fixed run remains the rollback baseline. | Release records and the current Full matrix validate it. | Every experiment has a production default or feature switch. |
| Human evidence is required to judge fun. | Existing automated checks intentionally cover correctness, not enjoyment. | No acceptance claim relies only on telemetry or test counts. |
| Input defaults may change during experiments. | The active keyboard-only contract is valid but does not resolve the reported attack-agency problem. | Defaults remain conflict-free and saved bindings receive migration fixtures. |
| Stable state and reward owners should not be rewritten. | Current exactly-once and retry contracts are already proven. | Refactors delegate behind existing interfaces and compare golden behavior. |

## Proposed Design

Deliver the work as reversible vertical hypotheses: record the current baseline,
extract only the seams needed for experiments, test three attack-intent models,
add readable interaction states and build loops, then prove them in Ruin Approach
and the Slime King before deciding whether broader content or generation resumes.

## As-Is / To-Be Delta Map

| Concern | As-is | To-be | Accept | Guard |
| --- | --- | --- | --- | --- |
| Experience evidence | Correctness-heavy release matrix; no reconstructable play timeline. | Development-only event record plus repeatable movement/combat fixtures. | A reviewer reconstructs intent, damage, route, and retry events. | Recorder cannot affect saves, rewards, transitions, or release builds. |
| Combat ownership | Large controller owns input, intent, execution, defense, resources, and feedback. | Narrow delegates behind the existing orchestrator. | Golden snapshots and continuous play show no unintended delta. | No new generic event bus or parallel production owner. |
| Attack agency | One contextual action selects melee or ranged. | Compare forecasted contextual, tap/hold, and dedicated melee/ranged actions. | Prediction, accidental spend, response time, accessibility, and confidence gates select one. | Remove losing experiment paths after integrated regression. |
| Combat/build depth | Mostly isolated enemy questions and several numerical rewards. | Shared openings and Pursuit/Counter/Control interaction loops. | Players recognize states and builds from behavior. | Bounded triggers prevent infinite resource or stagger loops. |
| Stage/boss | Valid complete run with limited continuous-play memory evidence. | One memorable Ruin proof path and a boss that tests its learned verbs. | Route recall, pacing, boss comprehension, and replay gates pass. | Existing fixed plan remains available until acceptance. |
| Presentation/generation | Mature shell, prototype action scenes, dormant random topology. | Final-enough proof-path readability, then staged procedural re-entry only after Go. | Human clarity and enjoyment match the fixed benchmark. | Later-stage art and broader generation remain blocked. |

## Progress

- PR 0's four-document review package has been integrated locally and repaired to
  match repository lifecycle, link, and plan-structure rules.
- No runtime milestone in this roadmap has started.
- The plan remains draft until the owner accepts the Core Play Proof scope; the
  active PRD and production contracts continue to govern current behavior.

## Next Steps

1. Review the Core Play Proof scope, three attack-intent candidates, and human
   acceptance gates.
2. If accepted, change this plan and the foundation spec to active in a dedicated
   decision commit without superseding unrelated active specs.
3. Begin PR 1 with the recorder and baseline fixtures; do not start mechanics
   changes before baseline evidence exists.
...
| Question | Experiment |
| --- | --- |
| Should one attack button remain contextual? | Compare forecasted contextual, tap-melee/hold-ranged, and dedicated melee/ranged actions in identical fixtures. |
...
The ordered pull requests below are the roadmap milestones. A later milestone may
start only after the prior automated gate passes and its human result is recorded.
...
**Primary hypothesis:** Combat experiments can be implemented safely only after the large production controller exposes narrow owners.
...
- implement development-selectable Variant A, Variant B, and Variant C from `CORE_PLAY_FOUNDATIONS.md`;
...
- Inner loop: run the narrow fixture or validator owned by the changed subsystem.
- PR gate: run the affected gameplay, persistence, UI, and stage contracts plus
  `git diff --check` and task-owned document/link validation.
- Final gate: run the Full release matrix and production Web export before each
  player-facing behavior PR merges.
- Human gate: record the PR-specific observation before enabling an experiment by
  default; automation alone cannot close an experience hypothesis.
- Rerun a failed broad gate only after a concrete fix or new diagnosis.

The integrated proof additionally covers:
...
- Preserve the accepted fixed run and current production defaults until a human
  gate selects a replacement.
- Keep experiments behind development settings or independently removable feature
  switches; never require experimental state to load an existing profile.
- Avoid save-schema changes. If one becomes necessary, stop for explicit owner
  approval and add forward, rollback, and corruption fixtures first.
- Land seam extraction separately from behavior changes and compare deterministic
  snapshots before tuning.
- Remove losing experiment paths only after the chosen path survives a later
  integrated regression cycle.
- A No-go result returns work to the failed category; it does not trigger content
  expansion or deletion of the validated baseline.

## Risks

### Risk: The contextual attack is fundamentally frustrating

Mitigation: select among contextual, tap/hold, and dedicated melee/ranged actions
using the same fixtures and evidence. Do not preserve the one-button constraint if
the dedicated variant is materially clearer.
...
- Are contextual, tap/hold, and dedicated melee/ranged actions the right three
  bounded candidates for the experiment?
