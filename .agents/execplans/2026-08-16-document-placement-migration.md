...
| Move to `.agents/research/performance/` | `.agents/2026-08-02-enemy-movement-spawn-research.md`; `.agents/2026-08-11-enemy-scale-performance-research.md`; `.agents/cardborne-runtime-architecture-audit.md`; `.agents/combat-clarity-runtime-difficulty-analysis.md`; `docs/performance/2026-08-13-dense-enemy-architecture-options.md`; `docs/performance/2026-08-13-enemy-arrival-and-engagement-research.md` | Reusable codebase and performance synthesis; advisory, not policy |
| Move to `.agents/research/reports/` | `docs/reports/2026-08-11-vehicle-upgrade-idea-catalog.md`; `docs/reports/2026-08-13-upgrade-artwork-reference-analysis-ko.md`; `docs/reports/assets/upgrade-artwork-reference-analysis/**`; `docs/reports/2026-08-14-game-telemetry-and-feedback-research.md`; `docs/reports/2026-08-14-upgrade-category-language-review.md`; `docs/reports/2026-08-15-eight-boss-combat-design-analysis.md`; `docs/reports/2026-08-16-boss-pattern-density-research.md` | Reusable design and reference analysis, including the artwork study's required source bundle |
| Move to `.agents/evidence/performance/` | `docs/performance/2026-08-13-dense-enemy-stutter-evidence.md`; `docs/performance/2026-08-13-dense-enemy-conclusion-ko.md`; `docs/performance/2026-08-13-enemy-arrival-conclusion-ko.md`; `docs/performance/vehicle-performance-evidence.jsonl`; `docs/performance/evidence/**`; `.agents/semantic-v2-runtime-acceptance-evidence.md` | Measurements, conclusions tied to captures, and runtime acceptance proof |
| Move to `.agents/evidence/reports/` | `docs/reports/2026-08-11-anomaly-device-runtime.md`; `docs/reports/2026-08-12-player-facing-terminology-audit.md`; `docs/reports/2026-08-15-combat-readability-pressure-review.md`; `docs/reports/2026-08-15-eight-boss-combat-approval-ko.md`; `docs/reports/2026-08-16-boss-pattern-density-problem-brief.md`; `docs/reports/game-system-review/effects-upgrades-as-is.md` | Audits, approvals, and observed-state evidence |
...
- [ ] **1.1 Update repository guidance.** Add the `docs` / `.agents/research` / `.agents/evidence` authority boundary to the root and `.agents` guidance without adding a full directory inventory.
...
- [ ] **1.2 Capture a source manifest.** Record each source path and SHA-256 before moving bundles.
  - Accept: every listed source exists and the manifest has one unique target per source.

### Phase 2: Move the locked document sets

- [ ] **2.1 Move performance research and evidence with `git mv`.** Preserve the nested `docs/performance/evidence/**` structure under `.agents/evidence/performance/evidence/**`.
  - Accept: every old path is absent, every mapped target exists, and target hashes match the manifest.
- [ ] **2.2 Move report research and evidence with `git mv`.** Preserve any companion assets required by a listed document; do not touch the visual workbench.
...
- [ ] **3.1 Update frontmatter and inbound links.** Repair `related`, `supersedes`, and ordinary Markdown references in moved files and their consumers, including performance reports and upgrade/boss report cross-links.
  - Accept: no tracked file references a moved old path; all changed relative links resolve.
- [ ] **3.2 Preserve authority.** Update indexes only where they enumerate moved files; do not promote any research conclusion into a spec in this migration.
  - Accept: canonical product/design files are unchanged except for necessary link updates.

### Phase 4: Validate and close

- [ ] **4.1 Run the final document checks.** Run `git diff --check`, `git diff --name-status -- docs .agents AGENTS.md`, and targeted `rg -n --hidden` searches for every old path stem.
  - Accept: checks pass, no runtime path is changed, and all moved targets match the source manifest.
- [ ] **4.2 Close this contract.** Record concise evidence, set `status: done`, and commit only migration-owned files.
...
- Next task after approval: 1.1 Update repository guidance.
- Last completed gate: repository-wide file and link audit; dispositions above are locked.
