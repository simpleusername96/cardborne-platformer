...
- [x] Removed Stage 8's wedge-ring warning and damage zones; its delayed radial selections now reveal only twelve live projectile bodies when each volley fires.
...
Stage 8 projectile-effect correction checkpoint (2026-08-21): the former `alternating_pulse` contract combined a visible wedge-ring damage zone with a delayed twelve-shot radial volley. The canonical pattern IDs and kind are now `radial_volley_a` / `radial_volley_b` and `radial_volley`. `VehicleBossRuntime` owns a fixed-capacity four-entry delay queue; `VehicleRun` retains only the projectile-allocation side effect. The ring collision and renderer branches are removed, authored projectile damage is preserved, and only twelve live hostile projectile bodies appear after each delay. Product and visual authority prohibit every Stage 8 radial-volley pre-fire world effect. Boss pattern, boss runtime, late-boss identity, twelve-boss campaign, guidebook, renderer, integrated run, route-readability, late-boss mechanics, and visual-authority validators pass. The post-pass found no competing schedule owner or reachable hidden-damage path. No release-performance claim is made from these focused checks.

Visual-authority receipt (2026-08-21): complete read of `docs/design/VISUAL_SYSTEM.md` at SHA-256 `86e9ab2549793c70c55edf1b1c9856a9811a2a433c445a4f78b221238d30caa2`; original-detail inspection of `docs/design/cardborne-universal-art-style-reference.png` at required and observed SHA-256 `96ccf5d053e66dd3a102ccdf39daefd0b0c54b0e88d20428b7ba1c894f002889`. Original artifact provenance remains `C:/Users/BK/.codex/generated_images/019fbfe9-857e-7453-b72d-20908d848577/exec-0b8aa606-cf55-45c1-abb3-fb3df762b080.png`, timestamp 2026-08-02 12:13:44 KST. Task constraint: Stage 8 projectile emission has no warning zone, route, impact, source, or other pre-fire world effect; only the authored hostile projectile body is visible after spawn. `actual_image_reference_used=false`, `reference_input_method=not_applicable`; no raster was created or edited and no asset approval changed.
...
- [x] Make every hostile projectile startup descriptor source-only: retain origin, committed direction, damage, affinity, lead time, and readiness, but never publish a predicted endpoint, corridor width, or future path. This includes the stage-8 broad barrage.
- [x] Remove Stage 8 radial-volley warning and damage zones entirely; schedule the delay without visual geometry and show only the twelve live projectile bodies after emission.
...
Acceptance: no bombardment can damage during warning; sampled center, middle, and edge points receive the exact locked scales; projectile visuals cover collision truth; no hostile projectile startup descriptor or renderer exposes predicted route geometry; Stage 8 radial volleys publish no warning or damage area and reveal only live projectile bodies; off-screen source readiness remains available; actual active beams and non-projectile damage areas remain visible; ordinary and boss attack commits increase without reducing source readability.
