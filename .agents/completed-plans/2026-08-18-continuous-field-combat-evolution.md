...
- Replace Stage 7 Boss and Stage 8 Boss's ineffective direct-pattern entries with executable, collision-truthful mechanics and retain their existing boss PNG identities.
...
- Stage 7 Boss identity is a collision-true crossing weave: paired translating walls with one readable gap are followed by an orthogonal pass whose gap is on a different axis. Stage 8 Boss identity is an alternating safe-sector pulse followed by a rotating sparse radial volley. Both must run through explicit direct and autonomous handlers and keep the documented escape-margin rules.
...
### Phase 3: Give Stage 7 Boss and Stage 8 Boss executable identities
...
- [x] **3.1** Implement Stage 7 Boss's crossing weave.
  ...
  - Accept: both direct and autonomous sequences execute it; safe gaps and warning geometry match damage geometry; no reused Cross Beam remains in Loom's direct identity slots.
- [x] **3.2** Implement Stage 8 Boss's alternating pulse.
