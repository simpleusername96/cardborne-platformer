...
| Archive | Reposition through orthogonal/X geometry | Cross Beam, Archive Depth, Current Fan | Undertow Lanes, Depth Charges | Make the X two exact committed corridors; do not let an autonomous area erase the readable crossing gap. |
| Stage 3 boss | Flank a directional defense, then respect stored counterpressure | Grounding Grid, Stage 3 boss Counterburst, Stage 3 boss Pulse | Thunder Chain, Fixed Beam Ordinary Enemy Lv.1 | Align the rendered frontal arc with the interception angle; communicate counterburst charge on the body/HUD, not with a ground route. |
...
| Stage 5 boss | Choose and break a shield sector while lanes close | Stage 5 boss Beam, Mirror Cross, Carrier Wave | Stage 5 boss Lattice, Relay Pulse Rings | Align each body sector with collision truth and keep broken sectors visibly absent; phase adds should reinforce, not hide, sector choice. |
...
| Loom | Find and follow translating gaps | Translating Walls, Cross Beam, Orthogonal Pass | Moving walls | Create collision-true wall segments during direct startup, retain the 180-unit gap, and prevent a second wall system from closing the authored escape at the same time. |
