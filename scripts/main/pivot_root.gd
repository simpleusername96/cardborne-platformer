...
func _register_input_map() -> void:
	...
	_add_action("melee", [_key(KEY_SPACE), _joy_button(JOY_BUTTON_RIGHT_SHOULDER)])
	_add_action("ranged", [_key(KEY_Z), _joy_axis(JOY_AXIS_TRIGGER_RIGHT, 1.0)])
	_add_action("dash", [_key(KEY_SHIFT), _joy_button(JOY_BUTTON_A)])
	_add_action("interact", [_key(KEY_X), _joy_button(JOY_BUTTON_X)])
