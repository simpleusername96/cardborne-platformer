...
- Image quality and asset fidelity: all cards use existing semantic gameplay PNGs through `VehicleUpgradeGlyphRenderer`; no placeholder, generated runtime image, SVG substitute, code drawing, sprite-sheet crop, or stretched screenshot is used. The final renderer fills the assigned glyph slot while preserving aspect ratio and sharpness.
...
3. The first full-view comparison found P2 under-scaling of semantic glyph content inside the correct control slot. `VehicleUpgradeGlyphRenderer` changed from 92% to 100% slot fill without enlarging the control or narrowing the comparison lane. The final comparison shows larger, sharp glyphs and both layout validators still pass.
