...
func _initialize() -> void:
	...
	_expect(Catalog.cover_rects().size() == 13, "field has thirteen blockers")
	...
	_expect(Catalog.ordinary_spawn_anchors().size() == 16, "field has sixteen ordinary anchors")
	_expect(Catalog.boss_arrival_anchors().size() == 8, "field has eight boss anchors")
	for anchor in Catalog.ordinary_spawn_anchors():
		_expect(Rules.is_position_walkable(anchor, 36.0, &"stage_1"), "ordinary anchor is walkable")
		_expect(Rules.grid_reachable(Catalog.player_start(), anchor, 36.0, 96.0, false, &"stage_1"), "ordinary anchor reaches center")
	for anchor in Catalog.boss_arrival_anchors():
		_expect(Rules.is_position_walkable(anchor, 76.0, &"stage_1"), "boss anchor is walkable")
		_expect(Rules.grid_reachable(Catalog.player_start(), anchor, 76.0, 96.0, false, &"stage_1"), "boss anchor reaches center")
	_check_stage_flow()
	_finish()
