...
func _initialize() -> void:
	...
	_expect(
		int(gameplay_manifest.get("final_asset_count", 0)) == 117,
		"gameplay manifest declares 114 approved semantic PNGs plus three approved SurfaceDetail SVGs"
	)
...
func _validate_active_world_catalog() -> void:
	...
	var expected := [
		...
		&"enemy_upgrade_device", &"transit_gate", &"repair_beacon",
	]
	...
	_expect(
		matches,
		"runtime world catalog exposes the hostile device, gate, and retired facility identities"
	)
