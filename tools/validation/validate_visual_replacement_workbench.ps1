...
try {
    ...
    Expect ($actual.summary.units -eq 31) 'switch unit count must be 31'
    Expect ($actual.summary.retire_only -eq 8) 'retire-only count must be 8'
    Expect ($actual.summary.gameplay_png -eq 107) 'production gameplay PNG count must be 107'
    Expect ($actual.summary.final_gameplay_png -eq 116) 'final gameplay PNG forecast must be 116'
    ...
    Expect ($actual.summary.gameplay_images -eq 110) 'production gameplay image count must be 110'
    Expect ($actual.summary.authored_gameplay_png -eq 114) 'authored gameplay PNG output count must be 114'
    ...
    $expectedGameplayUnits=[ordered]@{
        ...
        eight_boss_ordinary_enemy_expansion=@(4,4)
        eight_boss_body_expansion=@(3,3)
        eight_boss_shared_death_explosion=@(0,0)
        eight_boss_support_facilities=@(2,2)
        eight_boss_primary_upgrade_cards=@(3,3)
    }
