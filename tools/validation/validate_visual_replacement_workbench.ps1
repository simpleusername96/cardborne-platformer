...
try {
    ...
    Expect ($actual.summary.gameplay_png -eq 59) 'production gameplay PNG count must be 59'
    Expect ($actual.summary.final_gameplay_png -eq 59) 'final gameplay PNG forecast must be 59'
    Expect ($actual.summary.authored_gameplay_png -eq 57) 'authored gameplay PNG output count must be 57'
    ...
    $expectedGameplayUnits=[ordered]@{
        ...
        gameplay_code_asset_rasterization=@(10,10)
    }
...
foreach($required in @('id="language-toggle"','id="search"','id="domain-filter"','id="status-filter"','id="kind-filter"','id="issue-only"','id="copy-issues"','id="download-issues"','data-issue-check','data-issue-note','localStorage','cardborne.visualReplacementIssues.v1','Technical status','기술 상태','<dialog id="image-dialog"','loading="lazy"','prefers-reduced-motion','data-image','aria-live="polite"','approved_for_switch','target_required','retire_only','"final_gameplay_png":59','"external_sources"','"style_reference_sheet"','"visual_authority_evidence"')){Expect ($index.Contains($required)) "index contract missing: $required"}
$authorityUiMarkers=@(
