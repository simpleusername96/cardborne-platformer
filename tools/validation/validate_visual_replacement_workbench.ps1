...
try {
    ...
    Expect ($actual.summary.gameplay_png -eq 71) 'production gameplay PNG count must be 71'
    Expect ($actual.summary.final_gameplay_png -eq 71) 'final gameplay PNG forecast must be 71'
    Expect ($actual.summary.authored_gameplay_png -eq 69) 'authored gameplay PNG output count must be 69'
    ...
    $expectedGameplayUnits=[ordered]@{
        ...
        hazard_and_mystery_device_family=@(4,4)
...
foreach($required in @('id="language-toggle"','id="search"','id="domain-filter"','id="status-filter"','id="kind-filter"','id="issue-only"','id="copy-issues"','id="download-issues"','data-issue-check','data-issue-note','localStorage','cardborne.visualReplacementIssues.v1','Technical status','기술 상태','<dialog id="image-dialog"','loading="lazy"','prefers-reduced-motion','data-image','aria-live="polite"','approved_for_switch','target_required','retire_only','"final_gameplay_png":71','"external_sources"','"style_reference_sheet"','"visual_authority_evidence"')){Expect ($index.Contains($required)) "index contract missing: $required"}
$authorityUiMarkers=@(
