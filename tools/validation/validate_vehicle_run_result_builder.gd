...
func _init() -> void:
	...
	for stage_number in range(3, 13):
		records.append({
			"stage_number":stage_number,
			"has_boss":true,
			"has_next_stage":stage_number < 12,
	...
	unfinished_records[0]["boss_report"] = {"id":&"BOSS_STAGE_02", "cleanup_started":true, "cleanup_completed":false}
