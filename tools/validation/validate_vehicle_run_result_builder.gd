...
func _init() -> void:
	var records: Array = [
		{
			"stage_number":1, "has_next_stage":true,
		...
		{
			"stage_number":2, "has_next_stage":true,
		...
		{"stage_number":3, "has_next_stage":true, "defeats":[], "outgoing":[], "attributes":[]},
		{"stage_number":4, "has_next_stage":true, "defeats":[], "outgoing":[], "attributes":[]},
		{"stage_number":5, "has_next_stage":false, "defeats":[], "outgoing":[], "attributes":[]},
	]
	...
	_expect(bool(result.get("complete_run", false)) and int(result.get("final_stage_number", 0)) == 5, "final result requires an ordered terminal five-stage record")
