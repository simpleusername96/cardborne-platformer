...
func _init() -> void:
	var records: Array = [
		{
			"stage_number":1, "has_boss":false, "has_next_stage":true,
	...
	for stage_number in range(3, 11):
		records.append({
			"stage_number":stage_number,
			"has_boss":stage_number % 2 == 0,
			"has_next_stage":stage_number < 10,
	...
	_expect(bool(result.get("complete_run", false)) and int(result.get("final_stage_number", 0)) == 10, "final result requires an ordered terminal ten-stage record")
	_expect(int(result.get("boss_stage_count", 0)) == 5, "final result records five even-stage bosses")
