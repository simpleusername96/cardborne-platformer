...
func _initialize() -> void:
	...
	_validate_cycle_publication_and_retirement()
...
func _validate_cycle_publication_and_retirement() -> void:
	...
	_expect(
		...
		"Tutorial cycle 1 must publish no enemy upgrade devices."
	)
	var cycle_two_health := 0.0
	for cycle_number in range(2, 13):
		runtime.configure(
			_selection_blueprint(), 101, StringName("stage_%d" % cycle_number)
		)
		runtime.refresh_publication(Rect2(), Vector2.ZERO)
		var active := _active_snapshots(runtime)
		_expect(
			active.size() == DeviceRuntime.ACTIVE_DEVICES_PER_CYCLE,
			"Every non-tutorial cycle must publish exactly four devices."
		)
		_expect(
			_active_ids(active) == PackedStringArray([
				"test_device_1", "test_device_2", "test_device_3", "test_device_5",
			]),
			"Fixed input must select the farthest-first maximum-separation sockets."
		)
		if active.is_empty():
			continue
		if cycle_number == 2:
			cycle_two_health = float(active[0]["max_health"])
			var broken := runtime.receive_damage(
				StringName(active[0]["id"]), 99999.0, &"player", &"projectile"
			)
			_expect(bool(broken["broken"]), "Player damage must resolve an active device.")
			runtime.refresh_publication(Rect2(), Vector2.ZERO)
			_expect(
				_active_snapshots(runtime).size() == 3,
				"A resolved device must not republish in the same cycle."
			)
		elif cycle_number == 3:
			_expect(
				float(active[0]["max_health"]) > cycle_two_health,
				"Device health must scale from the current cycle index."
			)
...
func _validate_objective_field_budget() -> void:
	...
	for _tick in 80:
		...
		_expect(
			...
			"Four route fields must share the fixed 512-cell per-tick budget."
		)
		...
		if ready:
			break
	_expect(ready, "All four device route fields must converge around authored blockers.")
	for target_id in targets:
		_expect(
			fields.path_cost(StringName(target_id), Vector2(3600.0, 3600.0)) >= 0,
			"Each completed route field must retain a reachable path sample."
		)
	targets.erase(&"field_b")
	fields.update(targets, true)
	var reduced := fields.debug_snapshot()
	_expect(
		int(reduced["target_count"]) == 3
			and int(reduced["target_capacity"]) == 4
			and int(reduced["cell_capacity"]) == ObjectiveFieldSet.CELL_COUNT,
		"Removing one objective must release only that bounded field without resizing capacity."
	)
...
func _validate_collision_ordering_and_minimap_markers() -> void:
	...
	for marker_variant in Array(minimap["markers"]):
		if StringName(Dictionary(marker_variant)["kind"]) == &"mystery_device":
			marker_count += 1
	_expect(marker_count == 4, "All four active devices must publish minimap markers.")
	run.free()
...
func _active_ids(active: Array[Dictionary]) -> PackedStringArray:
	var result := PackedStringArray()
	for device in active:
		result.append(String(device["id"]))
	return result
