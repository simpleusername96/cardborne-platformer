...
func _initialize() -> void:
	...
	_expect(is_equal_approx(BossPatterns.startup_seconds("common_lane_volley", 0), 0.18), "rapid projectile startup is an explicit absolute value")
	_expect(is_equal_approx(BossPatterns.startup_seconds("common_parallel_beam", 0), 0.45), "broad beam startup retains its explicit readable value")
