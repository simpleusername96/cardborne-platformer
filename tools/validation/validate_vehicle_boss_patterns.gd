...
func _validate_common_progression() -> void:
	...
	_expect(
		...
		"Stage 1 teaches charge, broad barrage, and radial bombardment"
	)
	_expect(
...
func _validate_pattern_contracts() -> void:
	...
	_expect(
		is_equal_approx(Patterns.startup_seconds("thermal_gates", 0), 0.18)
...
func _validate_signature_ownership() -> void:
	...
	for stage_id in [&"stage_10", &"stage_11", &"stage_12"]:
		_expect(
			Patterns.common_sequence(stage_id).size() == 5,
	...
	_expect(
		Patterns.behavior_family("reflect_fan") == &"fan_volley"
			...
			and Patterns.is_shared_behavior("common_charge"),
		"projectile aliases share one family while common attacks remain cross-stage"
	)
	_expect(
...
func _validate_absolute_profiles() -> void:
	...
	_expect(
		Patterns.damage("common_broad_barrage", 11) > Patterns.damage("common_broad_barrage", 0)
