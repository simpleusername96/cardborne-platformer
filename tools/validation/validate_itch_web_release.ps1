...
# Inspect literal HTML resource attributes and Godot's generated runtime config.
# Arbitrary JavaScript strings can contain documentation text that resembles paths.
...
if ($totalGzipBytes -gt $MaxInitialPayloadGzipBytes) {
    throw "Web gzip payload is $totalGzipBytes bytes; the project limit is $MaxInitialPayloadGzipBytes bytes."
}
...
Write-Host 'itch.io Web release static verification passed.'
