...
func _run_leaper_sequence(label: String) -> Dictionary:
	...
	for frame in MAX_FIXTURE_FRAMES:
		...
		if warning_frame < 0 and snapshot["warning"]:
			...
			_expect(marker != null and marker.visible, "%s warning should show the landing arc" % label)
			_expect(
				marker != null and marker.points.size() == 3,
				"%s warning arc should show takeoff, apex, and landing" % label
			)
