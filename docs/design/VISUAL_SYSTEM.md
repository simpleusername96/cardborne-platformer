...
`SurfaceDetail` workflow below.** The separate shared boss-death explosion exception is
one approved authored RGBA raster made through grounded ImageGen; it does not authorize
SVG, ImageMagick drawing, frame animation, particles, or another effect raster. The
user-approved SurfaceDetail exception permits deterministic
...
- Boss death uses exactly one shared authored `256x256` RGBA explosion overlay. Runtime
  places one centered instance over the unchanged boss body, grows it from scale `0.20`
  to `1.20`, and fades overlay and body together. It is the only authored effect raster;
  no sprite sheet, per-boss variant, particle, fragment body, or additional burst image
  is permitted.
...
- final gameplay manifest가 정확히 90 image를 색인함: semantic PNG 87개와
  user-approved SurfaceDetail SVG 3개다. 공용 보스 사망 폭발 PNG 한 개 외의
  effect/cue raster는 0이다.
