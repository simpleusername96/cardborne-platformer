...
scope: Proposed Core Play Proof controls, movement, combat, builds, encounters, stage rhythm, boss testing, and player-experience gates
...
The Core Play Proof is evaluated against these player-facing requirements:
...
| Attack intent | X; V is ranged only in Variant C | West face; right trigger is ranged only in Variant C | Final melee/ranged intent model is selected by the control experiment gate below. |
...
| Potion | Q | Right shoulder | Uses a dedicated, conflict-free action in every control variant. |
| Pause / back | Escape | Menu button | Same meaning across gameplay and shell UI. |

Every action is remappable. Default bindings are conflict-free; adding a keyboard
layout or gamepad navigation alias cannot reuse the potion or attack action. The UI
automatically switches glyph sets based on the last meaningful input device and
avoids rapid flicker from stick noise.
...
Three variants must be implemented behind a development setting and tested with identical encounters.
...
### Variant C: dedicated melee and ranged actions

- Melee and ranged intent use separate remappable actions.
- The starting comparison uses `X`/West face for melee and `V`/right trigger for
  ranged; these are experiment defaults, not a permanent binding decision.
- Each action still validates target, line of sight, and supply before commitment.
- No automatic fallback silently changes one requested tool into the other.
...
Do not combine competing schemes into a permanent complex hybrid. Select one
model, document the evidence, and remove the losing experiment paths after a
later integrated regression pass.
