...
- Give every directional enemy one explicit presentation-facing result derived from its effective
  ...
  intentional spin; stationary mine/generator remain neutral; startup/active turret, pull_enemy, Beam
  Sentinel, Fixed Ranged Ordinary Enemy Lv.2, and boss use frozen committed direction; all other directional
...
- Replace the current dark-bordered `cue/beam_strip_9` raster with a tintable flat alpha mask and
  retune the existing two-plane startup/three-plane active composition for Fixed Beam Ordinary Enemy Lv.1 and boss
  straight beams.
...
- Expand the minimap's bounded semantic roles to distinguish field pickup, reward crate, Mystery
  Device, mobile enemy, fixed priority enemy, and boss by shape as well as
  color.
- Normalize minimap perceived size instead of treating each silhouette's bounding box as its scale:
  ...
  maximum span so its larger world footprint sits above pickup and below the radius-10 boss
  hierarchy. Keep all six roles and one retained mesh.
...
| Ordinary movement role truth | The basic algorithm exists: edge_enemys and contact roles approach; shooters hold 330-500, controllers 390-540, artillery 520-760, escorts 300-470, and support roles 430-620. Stationary roles do not move. | `VehicleRun._desired_enemy_velocity`; `VehicleEnemyArchetypes`; `VehicleEnemyUpdateSchedule` | Preserve these bands and role identities. Centralize them in `VehicleEnemyMovementPolicy` so callers request pursuit/standoff/escort/support intent without owning threshold tables. | 3.3-3.6 |
...
| Thermal Burst hit contract | Existing `_damage_enemies_in_radius` uses the spatial grid but also damages devices. Seeker burst already excludes its direct target. | `VehicleRun._update_projectile_buffer`; `VehicleRun._damage_enemies_in_radius` | Trigger once for every direct enemy hit by a non-reflected `player_primary` projectile, including split/piercing hits. Use spatial-grid query, radius `72/84/96`, flat damage `4/6/8`, exclude the direct target, damage targetable enemies including a nearby boss through normal shield rules, exclude structures/devices, and never chain or apply Toxin/Chill. | 4.2-4.4 |
...
| Straight boss beam awkwardness | Current 128x32 `cue/beam_strip_9` is a white rectangle with a dark perimeter. Renderer startup stretches it twice and active stretches it three times, producing nested framed bars instead of energy planes. The same owner serves Fixed Beam Ordinary Enemy Lv.1 and boss beams. | Original-detail asset inspection; workbench unit `gameplay_code_asset_rasterization`; `VehicleCombatRenderer._sync_beam_startup/_sync_active_beam` | Preserve the exact straight collision corridor and shared semantic ID, but replace the raster with a borderless tintable alpha mask. Startup is two flat planes; active is three. No glow, gradient, endpoint cap, frame, particle, or extra batch. | 6.5-6.7 |
| Boss beam concept evidence | The first ImageGen concept was rejected because it introduced panel repetition and soft glow. A corrected second concept shows the intended startup/active hierarchy on a plain floor, but remains preview-only and is not the final 128x32 strip. | Canonical sheet, current Stage 5 boss, and current beam strip supplied through `image_gen.referenced_image_paths` | Carry forward the corrected flat-plane hierarchy. Generate and approve an exact canvas/pivot/import-compatible strip before runtime replacement. | 6.5-6.7 |
...
| Performance fixture mismatch | Renderer capacity and visual contract are 26 health overlays, but the performance scenario and its validator previously required 50. Release batch threshold remains at most 50. | combat renderer, visual system, renderer validator, performance scenario and validator | Match the scenario fixture to the current renderer capacity of 26; retain the release threshold `batches <= 50`. A failing focused validator blocks all expensive samples. | 1.1 |
...
| Minimap role collapse | Snapshot creation emits pickups, crates, and intact Mystery Devices as `item`; mobile and fixed ordinary enemies as `enemy`; builder supports only item/enemy/boss plus player. The large neutral Mystery Device can therefore disappear into pickup semantics. Boss differs mainly by size. | `VehicleRun._minimap_snapshot/_runtime_minimap_snapshot`; `VehicleMinimapMeshBuilder`; UI glyph catalog | Use seven bounded roles: player, field pickup, reward crate, Mystery Device, mobile enemy, fixed priority enemy, and boss. Distinguish shape as well as semantic color; keep Mystery Device outcome hidden; use one notched command-magenta boss marker instead of arbitrary per-stage colors. | 8.1, 8.3-8.4 |
...
- [x] **1.1** Restore a truthful performance fixture contract.
  - Change: replace the stale required health-overlay capacity `50` with the renderer capacity
    in the scenario and its focused validator. The current capacity is `26`; do not change the
    release batch threshold `50`.
  ...
  - Evidence: the 2026-08-09 checkpoint required `Overlay_health == 28`; removal of the obsolete
    spawn structure on 2026-08-12 reduced the current fixture and production-replay contract to
    `Overlay_health == 26` while the combat-batch threshold remains `50`;
...
- [x] **3.3** Centralize role movement intent without changing combat timing.
  ...
  - Accept: every mobile archetype resolves exactly one family: pursuit (`ordinary_melee_01`, `edge_enemy`,
    `pull_enemy`, `ordinary_shield_01`, `ordinary_pulse_01`, mobile `ordinary_area_01`), standoff (`ordinary_ranged_01`,
    `shooter`, `controller`, `ordinary_growth_01`), escort (`ordinary_support_02`), or support
    (`ordinary_support_01`, `ordinary_support_03`). Fixed installations resolve stationary.
...
- [x] **3.6** Add deterministic movement oracles and rendered feel checks.
  ...
  - Evidence: the policy validator covers every family at midpoint, edge, outside-band, and
    ...
    `build/visual-captures/phase3-movement-feel-ko-v9` shows a edge_enemy travel `601.66` units and
...
- [x] **4.2** Apply bounded enemy-only burst damage on primary hits.
  - Change: put level values `radius=[72,84,96]`, `damage=[4,6,8]` in the element profile. On each
    ...
    excluding the direct target and all structures/devices. Do not scan the full enemy
...
- [x] **6.6** Generate and obtain approval for the borderless beam mask.
  - Change: with the canonical sheet supplied as an actual image reference, generate an exact
    ...
    source at original detail plus Fixed Beam Ordinary Enemy Lv.1 and boss startup/active AS-IS/TO-BE captures.
  ...
  - Progress: built-in ImageGen received the authority sheet, current strip, and Stage 5 boss as
    ...
    and includes dedicated Fixed Beam Ordinary Enemy Lv.1 and Stage 5 boss-beam startup/active AS-IS/TO-BE evidence. On
...
- [x] **6.7** Retune the existing beam planes without adding a batch.
  ...
  - Accept: startup reads non-damaging and active reads stronger; the full active width remains
    visible; Fixed Beam Ordinary Enemy Lv.1 and every boss beam use the same approved strip; combat/effect counts,
  ...
  - Evidence: commit `655e0ee2` promotes the approved bytes at the pinned SHA-256. Import and
    ...
    Sentinel and Stage 5 boss startup as a restrained two-plane corridor and active as a borderless
...
- [x] **8.3** Split minimap semantics without leaking hidden outcomes.
  - Change: emit and draw six roles: player, `field_pickup`, `mystery_device`,
    `mobile_enemy`, `priority_enemy`, and `boss`. Intact Mystery Device uses one neutral system-notched marker and
    never reveals its result; resolved/retired devices disappear. Fixed `turret`,
    `ordinary_fixed_ranged_02`, `ordinary_fixed_beam_01`, and `generator` use the priority-enemy marker. Boss uses
  ...
  - Accept: role is distinguishable in grayscale by silhouette: pickup lozenge, device neutral
    cut marker, mobile enemy wedge/round mass, fixed enemy square/cut mass, and notched boss
    command mass. No subtype beyond the locked roles is emitted.
...
- [ ] **9.12** Publish truthful enemy-facing intent.
  ...
  - Accept: edge_enemy, shooter, artillery, escort, Support Ordinary Enemy Lv.1, Support Ordinary Enemy Lv.3, idle turret,
    Fixed Ranged Ordinary Enemy Lv.2, boss, and other directional roles point their authored sharp/front end at the
...
- [ ] **9.14** Normalize minimap size by perceived polygon area.
  ...
  - Accept: computed pickup/crate outer areas are within 10%; both remain distinct at native size
    and grayscale; Mystery Device is larger than either but smaller than the boss; overlapping
    fixtures do not hide the player or priority roles. Validator asserts area/span hierarchy rather
    than only constant radii, and Korean/English captures include pickup, device, and boss together.
...
- [ ] **9.16** Increase current ordinary outgoing damage by exactly 30%.
  ...
  - Accept: effective stage multipliers are exactly
    `1.755/1.80765/1.8603/1.91295/1.9656`; representative Stage 1 edge_enemy `14` resolves to `24.57`,
...
- [ ] **10.2** Perform built-product gameplay and visual QA.
  - Change: use production-style native and built Web paths to check all five ordinary-health
    ...
    upgrades; one shield-up/down boss cycle for every boss; Fixed Beam Ordinary Enemy Lv.1 and boss beam
...
- Last completed gate: Phase 9A is complete in `cbd6d2d4`. Toxin/Cryo now use one shared
  ...
  `build/visual-captures/phase3-movement-feel-ko-v9` keep both actors and cover visible; the edge_enemy
  ...
  seven roles with no Mystery Device outcome field, fixed installations use the priority
  mass, and bosses use a command-magenta notched mass. Focused combat-cue, world-visual,
  HUD layout/presenter, localization, visual
