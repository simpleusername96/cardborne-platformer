...
변경 전 78개 제품 이미지에서 감전 이미지 1개를 없애고, 2026-08-15에
승인된 새 이미지 13개를 원본 바이트 그대로 반영했다.
...
현재 production manifest는 최종 목표인 **90개**다. 모든 새 이미지는 공식 스타일
참조 시트를 실제 입력으로 만들어졌으며, 사용자가 아래 13개 파일과 SHA-256을
명시적으로 승인한 뒤에만 제품 경로로 복사했다.
...
사용자는 2026-08-15에 아래 13개 파일과 SHA-256 전체를 명시적으로 승인했다.
production 파일의 해시도 모두 아래 값과 일치한다.
