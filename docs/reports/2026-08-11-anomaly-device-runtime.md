...
같은 결과를 재현하며, 전투 중 결과를 다시 뽑지 않는다. 네 효과는 모두
플레이어에게 유리하고 보스·고정 구조물에는 중력·냉각·유인이 적용되지 않는다.
...
`revealed_outcome`을 남긴다. 장치 주변의 현재 유효 대상 수는 5Hz로만
갱신한다.

- 탄막 소거: 실제 범위 안 적 투사체 수
- 중력 견인·급속 냉각·유인 신호: 실제로 효과를 받을 수 있는 일반 적 수

이 숫자는 정보만 제공한다. 결과, 범위, 효과 지속 시간이나 발동 조건을 바꾸지
않는다. 상황에 따라 대상 수가 0일 수 있으며 자동 대기나 재추첨은 하지 않는다.
