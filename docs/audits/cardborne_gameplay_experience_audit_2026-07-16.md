...
canonical_for: Cross-discipline diagnosis of Cardborne's current play experience and improvement priorities
...
Prototype two attack-control variants in the same combat fixture:
...
Create one combat dojo containing Walker, Charger, Shooter, and Shield Guard fixtures plus a two-enemy mixed encounter. Run both input variants and the shared-state pass with identical stats.
...
1. `PlayerCombatController.gd` is a 2,348-line concentration point for input, phases, target resolution, defense, loadout resources, cards, Spirit effects, projectiles, feedback snapshots, and legacy compatibility.
...
3. A/B test contextual versus tap/hold attack intent in a combat dojo.
