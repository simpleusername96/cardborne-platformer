...
현재 카탈로그는 카드 13장, 선택 가능한 레벨 상태 36개로 구성된다.
...
| EMP | 피해 62, 피해·기절 반지름 285, 적 탄환 제거 반지름 325, 기절 2.1초, 준비 0.42초, 재사용 대기시간 13초 |
| 차체 | 최대 내구도 120, 이동 속도 280 px/s |
| 흡혈 | 실제 적용 피해의 0.5%, 회복 예산 최대 6, 초당 6 보충 |
...
첫 획득 설명은 기존 역할 요약을 사용한다. 이후 레벨은 실제 강화 방향을
구분해 열폭발은 `폭발 피해·범위 증가 / Stronger, wider burst`, 독은
`중독 피해·지속 증가 / Stronger, longer toxin`, 냉기는
`감속·지속 증가 / Stronger, longer chill`로 표시한다.
...
| `lifesteal` | 흡혈 / Lifesteal | 2 | 기본 0.5%에서 총 회복률을 높인다. 적에게 실제로 적용된 플레이어 소유 피해를 기준으로 L1: 총 2%, L2: 총 3.5%만큼 선체를 회복한다. 회복 예산은 최대 6이며 초당 6씩 보충되고, 현재 최대 선체를 넘지 않는다. |

## 보상과 적용 규칙

전체 카드에는 36개 레벨 상태가 있다. 선택 보조 무기 세 개 중 두 개만 획득하고
...
- `unlock`은 없던 동작을 처음 만드는 제안이다. 확산 총구, 관통 탄환, 선택형
  보조 무기 세 개와 속성 세 개의 첫 획득에 적용한다.
- `enhance`는 이미 보유한 동작을 강화하는 제안이다. 추적 미사일은 기본 Seeker가
  이미 장착되어 첫 카드부터 `enhance`이며, 모든 동작 카드의 이후 레벨도
  `enhance`다. 이 구분은 frozen snapshot과 접근성 이름에만 표시한다.
- 모든 합법 제안 상태는 실제 gameplay owner가 제공하는 effect row를 한 개 또는
  두 개 가진다. 선택형 보조 무기와 속성의 첫 획득은 존재하지 않던 현재값을
  `0`으로 꾸미지 않고 획득값만 표시한다.

| 카드 | Effect row 1 | Effect row 2 | 표시 수열 |
| --- | --- | --- | --- |
| 확산 총구 / Split Muzzle | 발사당 투사체 / Projectiles per volley | 일제 총 피해 / Total volley damage | `1->2->3`; `100%->140%->165%` |
| 관통 탄환 / Piercing Rounds | 추가 관통 / Additional penetrations | 없음 | `0->1->2->3` |
| 추적 미사일 / Homing Missiles | 발사당 미사일 / Missiles per volley | 미사일당 피해 / Damage per missile | `1->2->3`; `25->28->32` |
| 전기장 / Electric Field | 초당 피해 / DPS | 반지름 / Radius | 첫 획득 `8`, `120`; 이후 `8->12->16`, `120->140->160` |
| 회전 날개 / Orbiting Blades | 날개 수 / Blade count | 날개당 피해 / Damage per blade | 첫 획득 `2`, `14`; 이후 `2->3->4`, `14->18->22` |
| 후방 기뢰 / Drop Mines | 피해 / Damage | 투하 간격 / Deployment interval | 첫 획득 `48`, `3.2 s`; 이후 `48->60->72`, `3.2->2.8->2.4 s` |

카드의 세로 순서는 분류, 제목, 의미 이미지 한 개, 레벨, effect row 한두 개,
최대 두 줄 요약이다. UI는 이 값을 다시 계산하지 않고 frozen gameplay snapshot만
표시한다. 가로 separator, 변화 종류 문장, 별도 unlock 아이콘은 없다.
...
41장 카탈로그에서 삭제됐던 `siphon_matrix`의 2레벨 피해 비례 회복 규칙은
`lifesteal`로 복원했다. `흡혈 행렬 / Siphon Matrix`처럼 효과와 관계없는
의사기술어는 되살리지 않았다.
...
- 카드 리소스가 정확히 13장, 레벨 상태가 정확히 36개다.
- 카드 없이도 실제 적용 피해의 0.5%를 선체로 회복한다. `흡혈 / Lifesteal`
  L1/L2는 총 회복률을 2%/3.5%로 높이며, 모든 상태가 초당 6 회복 예산과
  최대 선체 한도를 지킨다.
- 분류별 카드 수는 주무기 2, 보조 무기 4, 공격 속성 3, 차체 및 지원 4이다.
- `수거 범위 / Pickup Radius`가 기존 3레벨 수집 효과와 +210 최종 보너스를 유지한다.
- 선택 보조 무기 ID는 `electric_field`, `orbiting_blades`, `drop_mines`이며 최대
  두 개만 획득한다. 기본 장착 추적 미사일을 합쳐 동시에 활성화되는 보조 무기는
  최대 세 종류다.
- 합법 ID가 세 개 이상인 모든 선택 경로에서 서로 다른 합법 카드 세 장을 제안한다.
- 모든 레벨의 한국어·영어 분류, 제목과 실제 수치 변화가 해석된다.
- 36개 합법 제안 상태가 모두 한두 개의 실제 effect row를 제공하고, 기본 장착
  추적 미사일의 첫 카드는 `enhance`, 선택형 보조 무기와 속성 첫 카드는
  `unlock`으로 판정된다.
