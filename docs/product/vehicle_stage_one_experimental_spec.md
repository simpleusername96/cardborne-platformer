...
The vehicle has 120 health and responsive strafe movement. Primary input is one press per attack; holding never repeats. Attack energy recovers linearly over three seconds, becomes usable at 34%, and is fully consumed by a shot. The Repeater gains higher damage and one pierce at full energy; the Scatter Array grows from three to five projectiles at full energy. Every shot produces a muzzle cue, projectile trail, cover hit, target impact, hit response, and generated sound.
...
The HUD shows health, stage and objective, boss state and health, selected/aimed target health, primary attack energy and resulting power tier, passive cadence, dash cooldown, EMP cooldown, active buffs, and a fogged minimap with discovered route objectives, rewards, field boss, and stage boss.
...
The combat safe frame anchors hull state at upper left, one concise objective at upper center, the explored minimap at upper right, a unified action rail at bottom center, and the current target at lower right. The action rail gives primary attack energy and its unavailable/quick/charged/full tier more width than passive or cooldown actions. During the stage boss, one boss strip replaces the ordinary objective and minimap instead of stacking duplicate information.
